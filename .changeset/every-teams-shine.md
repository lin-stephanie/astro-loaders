...
Relax Release identifier validation from fixed 16-character Base64 node IDs to GitHub global node IDs such as `RE_...`, reuse shared URL regexes, and document live entry identifiers as `string | object`.
...
Add `__typename` to the release GraphQL fragment and use it in `getValidReleaseNode()` so lookups only map real `Release` nodes before stripping the typename from returned data.

Return an `INVALID_IDENTIFIER` loader error when live release entry lookups by node ID, URL or `{ owner, repo, tagName }` do not resolve to a GitHub Release, instead of returning an empty result.
